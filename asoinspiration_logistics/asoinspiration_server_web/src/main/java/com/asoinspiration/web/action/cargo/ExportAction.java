package com.asoinspiration.web.action.cargo;
...
import com.asoinspiration.export.webservice.IEpService;
import com.asoinspiration.service.ContractService;
import com.asoinspiration.service.ExportProductService;
import com.asoinspiration.service.ExportService;
...
public class ExportAction extends BaseAction implements ModelDriven<Export> {
	 ...
	 * @throws Exception
	 */
	@Action(value = "exportAction_list", results = {
			@Result(name = "list", location = "/WEB-INF/pages/cargo/export/jExportList.jsp") })
	public String list() throws Exception {
		// TODO Auto-generated method stub
		org.springframework.data.domain.Page<Export> page2 = exportService.findPage(null,
				new PageRequest(page.getPageNo() - 1, page.getPageSize()));
	 ...
	 * @throws Exception
	 */
	@Action(value = "exportAction_toupdate", results = {
	...
	// exportService.saveOrUpdate(exportDb); //修改数据后保存数据库
	//
	// List<HashMap> returnList =
	// JSON.parseArray(returnMap.get("products").toString(), HashMap.class);
	//
	// for (HashMap hashMap : returnList) {
	// // 根据海关返回的货物id查询当前系统的货物对象，进行tax赋值
	// ExportProduct epDb =
	// exportProductService.get(hashMap.get("exportProductId").toString());
